...
- `generate_story_thumbnail_insight()` / `periodic_generate_story_thumbnail_insights()` - OpenAI Vision AI descriptions
- `generate_story_embedding()` / `periodic_generate_story_embeddings()` - pgvector embeddings from insights
...
Both `Story` and `Post` models have a pipeline of AI-enrichment fields, each populated by a Celery task:

1. **thumbnail_insight** (`TextField`) — OpenAI Vision API description of the thumbnail image. Generated by `generate_story_thumbnail_insight()`. Images are resized to 50% and JPEG-compressed at 60% before sending to reduce token usage.
...
3. **Similarity search** — `StorySimilarView` at `/instagram/stories/<id>/similar/` and `PostSimilarView` at `/instagram/posts/<id>/similar/` return ranked results using L2Distance. Requires the `pgvector` PostgreSQL extension.
